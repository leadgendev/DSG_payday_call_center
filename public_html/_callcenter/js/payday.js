...
lf.submitForm = function submitForm() {
    ...
    if (lf.isFormValid() == true) {
        ...
        $("#pay_day1").val(lf.getFormattedDate(dt, 1));
        dt = new Date($("#pay_date2").val());
        $("#pay_day2").val(lf.getFormattedDate(dt, 1));
        dt = new Date(lf.getCmbDate($("#bday_day")));
        $("#birth_date").val(lf.getFormattedDate(dt, 1));
        //simple moves
...
lf.bindEvents = function bindEvents() {
    ...
    $('#zip').blur(function() { lf.valReq(this, 'Zip is required', function(obj) { lf.valReg(obj, '[0-9]{5}', 'Zip is invalid - Zip code must be 5 Numbers'); }); });
    $('#ssn1').blur(function() { lf.valReq(this, 'SSN(Part 1) is required', function(obj) { lf.valReg(obj, '[0-9]{3}', 'SSN(Part 1) is invalid - SSN(Part 1) must be 3 Numbers'); }); });
    $('#ssn2').blur(function() { lf.valReq(this, 'SSN(Part 2) is required', function(obj) { lf.valReg(obj, '[0-9]{2}', 'SSN(Part 2) is invalid - SSN(Part 2) must be 2 Numbers'); }); });
    $('#ssn3').blur(function() { lf.valReq(this, 'SSN(Part 3) is required', function(obj) { lf.valReg(obj, '[0-9]{4}', 'SSN(Part 3) is invalid - SSN(Part 3) must be 4 Numbers'); }); });
    $('#phone_home1').blur(function() { lf.valReq(this, 'Home Phone (Part 1) is required', function(obj) { lf.valReg(obj, '[0-9]{3}', 'Home Phone (Part 1) is invalid - Home Phone (Part 1) must be 3 Numbers'); }); });
    $('#phone_home2').blur(function() { lf.valReq(this, 'Home Phone (Part 2) is required', function(obj) { lf.valReg(obj, '[0-9]{3}', 'Home Phone (Part 2) is invalid - Home Phone (Part 2) must be 3 Numbers'); }); });
    $('#phone_home3').blur(function() { lf.valReq(this, 'Home Phone (Part 3) is required', function(obj) { lf.valReg(obj, '[0-9]{4}', 'Home Phone (Part 3) is invalid - Home Phone (Part 3) must be 4 Numbers'); }); });
	...
	// New fields
	$('#hire_month').blur(function() { lf.valReg(this, '[0-9]{2}', 'Hire Month is invalid - Hire Month must be 2 Numbers'); });
    $('#hire_year').blur(function() { lf.valReg(this, '[0-9]{2}', 'Hire Year is invalid - Hire Year must be 4 Numbers'); });
	$('#payday_date1_date').blur(function() { lf.valReg(this, '[0-9]{2}', 'Pay Date One (Part 1) is invalid - Pay Date One (Part 1) must be 2 Numbers'); });
    $('#payday_date1_month').blur(function() { lf.valReg(this, '[0-9]{2}', 'Pay Date One (Part 2) is invalid - Pay Date One (Part 2) must be 2 Numbers'); });
    $('#payday_date1_year').blur(function() { lf.valReg(this, '[0-9]{4}', 'Pay Date One (Part 3) is invalid - Pay Date One (Part 3) must be 4 Numbers'); });
	$('#payday_date2_date').blur(function() { lf.valReg(this, '[0-9]{2}', 'Pay Date Two (Part 1) is invalid - Pay Date Two (Part 1) must be 2 Numbers'); });
    $('#payday_date2_month').blur(function() { lf.valReg(this, '[0-9]{2}', 'Pay Date Two (Part 2) is invalid - Pay Date Two (Part 2) must be 2 Numbers'); });
    $('#payday_date2_year').blur(function() { lf.valReg(this, '[0-9]{4}', 'Pay Date Two (Part 3) is invalid - Pay Date Two (Part 3) must be 4 Numbers'); });
	//
    ...
    $('#drivers_license').blur(function() { lf.valReq(this, 'Drivers License is required', function(obj) { lf.valReg(obj, '^([a-zA-Z0-9 _\\-\\.]+)$', 'Invalid characters found in Drivers License field.'); }); });
    $('#employer_name').blur(function() { lf.valReq(this, 'Employer name is required', function(obj) { lf.valReg(obj, '^([a-zA-Z0-9 _\\-\\.\\&]+)$', 'Invalid characters found in employer name field.'); }); });
    ...
    $('#phone_work1').blur(function() { lf.valReq(this, 'Work Phone (Part 1) is required', function(obj) { lf.valReg(obj, '[0-9]{3}', 'Work Phone (Part 1) is invalid - Work Phone (Part 1) must be 3 Numbers'); }); });
    $('#phone_work2').blur(function() { lf.valReq(this, 'Work Phone (Part 2) is required', function(obj) { lf.valReg(obj, '[0-9]{3}', 'Work Phone (Part 2) is invalid - Work Phone (Part 2) must be 3 Numbers'); }); });
    $('#phone_work3').blur(function() { lf.valReq(this, 'Work Phone (Part 3) is required', function(obj) { lf.valReg(obj, '[0-9]{4}', 'Work Phone (Part 3) is invalid - Work Phone (Part 3) must be 4 Numbers'); }); });
    $('#phone_work_ext').blur(function() { lf.valReg(this, '^[0-9]{1,5}$', 'Work Phone Extension is invalid - Work Phone Extension must be numeric'); });
    ...
    $('#drivers_license_st').change(function() { lf.valReq(this, 'Drivers license state is required'); });
    $('#own_home').change(function() { lf.valReq(this, 'Home owner question is required'); });
    ...
    $('#income_monthly').change(function() { lf.valReq(this, 'Monthly income is required'); });
    $('#bank_account_type').change(function() { lf.valReq(this, 'Bank account type is required'); });
    $('#months_at_bank').change(function() { lf.valReq(this, 'Months at Bank is required.'); });
	// New fields
	$('#hire_month').change(function() { lf.valReq(this, 'Hire Month is required.'); });
	$('#hire_year').change(function() { lf.valReq(this, 'Hire Year is required.'); });
	$('#payday_date1_date').change(function() { lf.valReq(this, 'Pay Date One Day is required.'); });
	$('#payday_date1_month').change(function() { lf.valReq(this, 'Pay Date One Month is required.'); });
	$('#payday_date1_year').change(function() { lf.valReq(this, 'Pay Date One Year is required.'); });
	$('#payday_date2_date').change(function() { lf.valReq(this, 'Pay Date Two Day is required.'); });
	$('#payday_date2_month').change(function() { lf.valReq(this, 'Pay Date Two Month is required.'); });
	$('#payday_date2_year').change(function() { lf.valReq(this, 'Pay Date Two Year is required.'); });
}
/* Calendar functions */
